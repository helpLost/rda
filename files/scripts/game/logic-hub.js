...
export function gameInit() {
    ...
    if (first == true) {
        ...
        if (civData.states.dev == false && civData.states.age == 'Tribal Era') {
            ...
            utilities.explanation("popupContainer", "startDialogue", `<p>NOTE: this message is temporary.</p><p>Your tribe has recently gained some notoriety in the area. You've been appointed the leader and given a mysterious, divine fruit with immortalizing powers. With your newfound rulerhood, will you lead your civilization to victory? Or will you let it fade into the veil of time as if it was never there?</p><button id = 'yes' onclick = 'document.querySelector("body").removeChild(document.getElementById("mask")); document.querySelector("#popupContainer").remove();' style = 'background-color: whitesmoke; border: 1px solid lightgrey; font-family: "Lucida Grande", "Lucida Sans"; padding: 5px 10px 5px 11px; border-radius: 5px;'>Ok</button>`, 0, 275, "width: 500px; text-align: center;");
        }
...
export function applyResearch(research_id, neededTech) {
    ...
    for (let item of elems) {
        if (item.charAt(0) != "$" && item.charAt(0) != "#") {
            for (let inner_item of Object.keys(civData.buildings.numbers.tribal_era)) {
                if (item == inner_item) {
                    document.getElementById(utilities.abbreviate(inner_item.toLowerCase()) + "Container").classList.remove("unlock");
                }
            }
        } else if (item.charAt(0) == "#") {
            document.querySelector(item).classList.remove("unlock");
        } else {
            // other things
        }
    }
